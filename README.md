...
本リポジトリのコードと文書は、Codexの支援を受けて作成しました。

**検証状況：** macOS上で、Quarto＋nbstata＋Stataによる `stata-demo.qmd` のHTML出力を確認しています。**Python・Rの分析手順および言語間連携の手順は未検証です。** 確認した環境と実行方法は、[共通のQuarto実行手順](docs/quarto-local-execution.md)を参照してください。
...
**python のバージョン管理は uv を前提**としているが、venv でも同様のことが可能。

Codex / Workのローカル環境で、権限制限やStataの画像出力によりレンダリングに失敗する場合は、[共通のQuarto実行手順](docs/quarto-local-execution.md)を参照してください。既存環境の指定、一時フォルダでの実行、成果物の回収と確認をまとめています。
...
本リポジトリのコードと文書は、[MITライセンス](LICENSE)で提供します。著作権表示とライセンス表示を保持することで、商用利用を含む利用・改変・再配布が可能です。内容は無保証で提供します。

Stata本体や依存パッケージなど、第三者が提供するソフトウェアには、それぞれのライセンスが適用されます。
